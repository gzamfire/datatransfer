...
io.sockets.on('connection', function(socket) {
    ...
    socket.on("updatedData", function(updatedData) {
        ...
        fs = require('fs');
        fs.writeFile("python/datasets/" + datasetID, JSON.stringify(updatedData), function (err, data) {
          if (err) {
    ...
    socket.on("get_file", function(seeking) {
        ...
        PythonShell.run('json_to_iim.py', options, function (err, results) {
          if (err) throw err;
    ...
    socket.on('send_data', function(data) {
        ...
        fs.writeFile("data.json", JSON.stringify(parsedData), function(err) {
            if(err) {
                ...
                PythonShell.run('json_to_iim.py', function (err) {
                    if (err) throw err;
